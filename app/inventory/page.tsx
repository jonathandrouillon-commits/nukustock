...
export default function Inventory() {
  ...
  const getSortedInventoryExportRows = () => {
    const rows = locationsWithStock.flatMap(
      (group) =>
        group.rows.map((row) => ({
          group,
          row,
    ...
    const compareRow = (
      ...
      const byCategory = () =>
        compareText(
          a.row.product.category,
          b.row.product.category
        )

      const bySubcategory = () =>
        compareText(
          a.row.product.subcategory,
          b.row.product.subcategory
        )
  ...
  const exportInventoryExcel = () => {
    ...
    const rows =
      getSortedInventoryExportRows().map(
        ({ group, row }) => ({
          Référence:
    ...
    XLSX.writeFile(
      workbook,
      `NukuStock-Inventaire-${activeInventory.id}.xlsx`
    )
  }
  ...
  const exportInventoryPdf = () => {
    ...
    doc.setFontSize(12)
    doc.text(
      `Inventaire ${activeInventory.id} - ${activeInventory.name}`,
      12,
      19
    )
    ...
    doc.text(
      `Date : ${new Date(
        `${activeInventory.date}T12:00:00`
      ).toLocaleDateString('fr-FR')} - Tri : ${
        EXPORT_SORT_OPTIONS.find(
          (option) =>
            option.value === exportSortMode
        )?.label || ''
      }`,
      12,
      24
      ...
      body:
        getSortedInventoryExportRows().map(
          ({ group, row }) => [
            ...
            row.product.name || '',
            group.location || '',
            row.theoreticalQty.toLocaleString(
              'fr-FR'
            ),
            row.realQty.toLocaleString(
              'fr-FR'
            ),
    ...
    doc.save(
      `NukuStock-Inventaire-${activeInventory.id}.pdf`
    )
  }
  ...
  const printInventory = async () => {
    ...
    for (const { group, row } of sortedPrintRows) {
      const category =
        String(
          row.product.category ||
            'Sans catégorie'
      ...
      if (category !== currentCategory) {
        ...
        printedRows.push(
          `<tr class="categoryRow">
            <td colspan="${selectedDefinitions.length}">
